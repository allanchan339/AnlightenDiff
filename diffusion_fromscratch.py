...
from utils.color_space import img2res, res2img, img_normalize, img_unnormalize
from utils.module_util import normalize_to_neg_one_to_one, unnormalize_to_zero_to_one
from utils.cond_utils import cond_data_transforms
...
class EnlightDiffusion(nn.Module):
    """
    EnlightDiffusion model implementing denoising diffusion probabilistic models.
    """
    ...
    @torch.no_grad()
    def sample(self, img_lr, encoder, return_all_timesteps=False):
        if self.config.sample_mode == 'ddpm':
            sample_fn = self.p_sample_loop
        else:
            NotImplementedError(f"sample mode {self.config.sample_mode} not implemented")
        return sample_fn(img_lr, encoder, return_all_timesteps)
    
    def p_sample_loop(self, img_lr, encoder,  return_all_timesteps = False):
        """
        ddpm sampling loop with center
        """
        ...
        cond = img_lr
        # 12 Channel 
        feature = cond_data_transforms(img_lr)
        center, feats_cond = encoder(feature) #must be [0,1] range
...
class LitDiffusion(pl.LightningModule):
    """
    The Lightning Module warp up for the EnlightDiffusion model.
    """
    ...
    def _on_valid_test_start(self):
        ...
        # import lpips
        # self.loss_fn_alex = lpips.LPIPS(net='alex').to(self.device).eval()
    ...
    def loss_fn(self, noise_pred, noise, x_start, t, img_lr, center):
        ...
        if self.hparams.loss_type == 'l1':
            ...
            loss = F.mse_loss(noise_pred, noise)
        elif self.hparams.loss_type == 'dfpl':
    ...
    def training_step(self, batch, batch_idxs):
        ...
        cond = img_lr
        if self.hparams.use_center:
            feature = cond_data_transforms(img_lr)
            center, feats_cond = self.encoder(feature)
            options = ['pred', 'ref']
    ...
    def sample(self, img_lr, return_all_timesteps=False):
        ...
        imgs = self.model.sample(img_lr, self.encoder, return_all_timesteps)  
        return imgs
