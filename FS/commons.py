...
#         torch._assert(self.head_dim * num_heads == self.embed_dim, "embed_dim must be divisible by num_heads")
...
#         torch._assert(embed_dim == self.embed_dim, "query's embed_dim must match: %d" % embed_dim)
#         torch._assert(list(query.size()) == [tgt_len, bsz, embed_dim], "query's size is unexpect")
